...
st.markdown("Upload a LAS or LAZ file and customize contour output.")

uploaded_file = st.file_uploader("Upload a LAS or LAZ file", type=["las", "laz"])

if uploaded_file:
    suffix = ".las" if uploaded_file.name.lower().endswith(".las") else ".laz"
    ...
    try:
        ...
        st.success(f"{suffix.upper()} file loaded successfully!")
    except Exception as e:
        st.error(f"Failed to read {suffix.upper()} file: {e}")
        st.stop()
